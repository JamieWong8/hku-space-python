...
try:
    ...
    if sample_data is not None:
        ...
        eval_data = {
            ...
            'funding_amount_usd': float(test_company.get('funding_amount_usd', 0)) if pd.notna(test_company.get('funding_amount_usd')) else 1000000.0,
            'valuation_usd': float(test_company.get('valuation_usd', 0)) if pd.notna(test_company.get('valuation_usd')) else 5000000.0,
            'team_size': int(float(test_company.get('team_size', 0))) if pd.notna(test_company.get('team_size')) else 10,
            'years_since_founding': float(test_company.get('years_since_founding', 0)) if pd.notna(test_company.get('years_since_founding')) else 2.0,
            'revenue_usd': float(test_company.get('revenue_usd', 0)) if pd.notna(test_company.get('revenue_usd')) else 100000.0,
            'num_investors': int(float(test_company.get('num_investors', 0))) if pd.notna(test_company.get('num_investors')) else 3,
